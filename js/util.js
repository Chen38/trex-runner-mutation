...
/**
 * Check the bullet collision.
 * @param {!Obstacle} obstacle
 * @param {!Bullet} bullet object.
 */
function checkBulletCollision(obstacle, bullet) {
  if (!bullet) {
    return false;
  }
  
  var obstacleBoxXPos = Runner.defaultDimensions.WIDTH + obstacle.xPos;
  var bulletBox = new CollisionBox(
    bullet.xPos + 1,
    bullet.yPos + 1,
    bullet.config.WIDTH - 2,
    bullet.config.HEIGHT - 2);
  var obstacleBox = new CollisionBox(
    obstacle.xPos + 1,
    obstacle.yPos + 1,
    obstacle.typeConfig.width * obstacle.size - 2,
    obstacle.typeConfig.height - 2);
  return boxCompare(bulletBox, obstacleBox);
}
/**
...
function throttle(fn, delay) {
  ...
  return function() {
    var excuteTime = Date.now();
    var remainingTime = delay - (excuteTime - startTime);
    var context = this;
    var argus = arguments;
    if (remainingTime <= 0) {
